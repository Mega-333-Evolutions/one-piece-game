...
async def manage(
    ...
    try:
        await dispatch_screens(event, context, command, user, inbound_keyboard)
    ...
    if screen is not Screen.UNKNOWN:
        ...
        match screen:
            case Screen.PVT_START:  # Start
                await manage_screen_start(event, context)

            case Screen.PVT_SETTINGS:  # Settings
                await manage_screen_settings(event, context, inbound_keyboard)

            case Screen.PVT_USER_STATUS:  # Status
                await manage_screen_status(event, context, command, user, inbound_keyboard)

            case Screen.PVT_CREW:  # Crew
                await manage_screen_crew(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_ABILITY:  # Crew Ability
                await manage_screen_crew_ability(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_ABILITY_ACTIVATE:  # Crew Ability Activate
                await manage_screen_crew_ability_activate(event, context, inbound_keyboard, user)
            ...
            case Screen.PVT_CREW_ABILITY_ACTIVATE_CONFIRM:  # Crew Ability Activate Confirm
                await manage_screen_crew_ability_activate_confirm(
                    event, context, inbound_keyboard, user
                )

            case Screen.PVT_CREW_CREATE_OR_EDIT:  # Crew Create or Edit
                await manage_screen_crew_create_or_edit(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_POWERUP:  # Crew Powerup
                await manage_screen_crew_powerup(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_LEVEL:  # Crew Level
                await manage_screen_crew_level(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_LEVEL_UP:  # Crew Level Up
                await manage_screen_crew_level_up(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_MODIFY:  # Crew Edit
                await manage_screen_crew_edit(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_LEAVE:  # Crew Leave
                await manage_screen_crew_leave(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_DISBAND:  # Crew Disband
                await manage_screen_crew_disband(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_MEMBER:  # Crew Member
                await manage_screen_crew_member(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_MEMBER_DETAIL:  # Crew Member Detail
                await manage_screen_crew_member_detail(event, context, inbound_keyboard, user)
            ...
            case Screen.PVT_POST_BAIL:  # Crew Member Post Bail
                await manage_screen_post_bail(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_SEARCH:  # Crew Search
                await manage_screen_crew_search(event, context, inbound_keyboard, user)

            case Screen.PVT_CREW_SEARCH_DETAIL:  # Crew Search Detail
                await manage_screen_crew_search_detail(event, context, inbound_keyboard, user)
            ...
            case Screen.PVT_SETTINGS_NOTIFICATIONS:  # Notifications
                await manage_screen_settings_notifications(event, context, inbound_keyboard)

            case Screen.PVT_SETTINGS_NOTIFICATIONS_TYPE:  # Notifications Type
                await manage_screen_settings_notifications_type(event, context, inbound_keyboard)
            ...
            case Screen.PVT_LOGS:  # Logs
                await manage_screen_logs(event, context, inbound_keyboard, user)

            case Screen.PVT_LOGS_TYPE:  # Logs Type
                await manage_screen_logs_type(event, context, inbound_keyboard, user)

            case Screen.PVT_LOGS_TYPE_DETAIL:  # Logs Type Detail
                await manage_screen_logs_type_detail(event, context, inbound_keyboard, user)

            case Screen.PVT_PREDICTION:  # Prediction
                await manage_screen_prediction(event, context, inbound_keyboard, user)

            case Screen.PVT_PREDICTION_DETAIL:  # Prediction Detail
                await manage_screen_prediction_detail(event, context, inbound_keyboard, user)
            ...
            case Screen.PVT_PREDICTION_CREATE:  # Prediction Create
                await manage_screen_prediction_create(event, context, inbound_keyboard, user)

            case Screen.PVT_DEVIL_FRUIT:  # Devil Fruit
                await manage_screen_devil_fruit(event, context, inbound_keyboard, user)

            case Screen.PVT_DEVIL_FRUIT_DETAIL:  # Devil Fruit Detail
                await manage_screen_devil_fruit_detail(event, context, inbound_keyboard, user)

            case Screen.PVT_DEVIL_FRUIT_DETAIL_EAT:  # Devil Fruit Detail Eat
                await manage_screen_devil_fruit_detail_eat(event, context, inbound_keyboard, user)
            ...
            case Screen.PVT_GAME_GUESS_INPUT:  # Guess game input
                await manage_screen_game_guess_input(event, context, inbound_keyboard, user)

            case Screen.PVT_LOGS_TYPE_STATS:  # Logs Type Stats
                await manage_screen_logs_type_stats(event, context, inbound_keyboard, user)

            case Screen.PVT_SETTINGS_TIMEZONE:  # Settings Timezone
                await manage_screen_settings_timezone(event, context, inbound_keyboard, user)

            case Screen.PVT_BOUNTY_LOAN:  # Bounty Loan
                await manage_screen_bounty_loan(event, context, inbound_keyboard, user)

            case Screen.PVT_BOUNTY_LOAN_DETAIL:  # Bounty Loan Detail
                await manage_screen_bounty_loan_detail(event, context, inbound_keyboard, user)

            case Screen.PVT_BOUNTY_LOAN_DETAIL_PAY:  # Bounty Loan Detail Pay
                await manage_screen_bounty_loan_detail_pay(event, context, inbound_keyboard, user)
            ...
            case Screen.PVT_CREW_DAVY_BACK_FIGHT:  # Crew Davy Back Fight
                await manage_screen_crew_davy_back_fight(event, context, inbound_keyboard, user)
            ...
            case Screen.PVT_DEVIL_FRUIT_SHOP:
                await manage_screen_devil_fruit_shop(event, context, inbound_keyboard, user)
            ...
            case Screen.PVT_FIGHT:
                await manage_screen_fight(event, context, user, inbound_keyboard)

            case Screen.PVT_PLUNDER:
                await manage_screen_plunder(event, context, user, inbound_keyboard)

            case Screen.PVT_DOC_Q_GAME:
                await manage_screen_doc_q_game(event, context, user, inbound_keyboard)

            case Screen.PVT_DAILY_REWARD:
                await manage_screen_daily_reward(event, context, user)

            case Screen.PVT_DAILY_REWARD_PRIZE:
                await manage_screen_daily_reward_prize(event, context, inbound_keyboard)

            case Screen.PVT_GAME_GLOBAL_LIST:
                await manage_screen_game_global_list(event, context, inbound_keyboard, user)

            case Screen.PVT_GAME:
                await manage_screen_game(event, context, user, command)

            case Screen.PVT_GAME_SELECTION:
                await manage_screen_game_selection(event, context, user, inbound_keyboard)
            ...
            case Screen.PVT_ROCK_PAPER_SCISSORS_GAME:
                await manage_screen_game_rps(event, context, user, inbound_keyboard)

            case Screen.PVT_RUSSIAN_ROULETTE_GAME:
                await manage_screen_game_rr(event, context, user, inbound_keyboard)

            case Screen.PVT_CHANGE_REGION:  # Change region
                await manage_screen_change_region(
                    event, context, user, keyboard=inbound_keyboard, command=command
                )

            case _:  # Unknown screen
                if event.callback_query is not None or screen is not None:
                    raise PrivateChatException(PrivateChatError.UNRECOGNIZED_SCREEN)

        user.last_system_interaction_date = datetime.now()
        await send_system_update_message(event, context, user)


async def send_system_update_message(
    event: Update, context: ContextTypes.DEFAULT_TYPE, user: User
) -> None:
    """
    Sends a system event message to the user
    :param event: Telegram event
    :param context: Telegram context
    :param user: User
    :return: None
    ...
    # Check if the user already received this system event
    ...
    # Don't send event if user joined after the event was made
    ...
    try:
        ...
        # at script end
        await full_message_send(
            context, ot_text, event=event, keyboard=inline_keyboard, new_message=True
        )
